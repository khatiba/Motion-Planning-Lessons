...
HBF::maze_path HBF::search(vector<vector<int>> grid, vector<vector<int>> heuristic, vector<double> start, vector<int> goal) {
  /*
  ...
  state.x = start[0];
  state.y = start[1];
  state.f = state.g + heuristic[idx(state.y)][idx(state.x)];
  state.theta = theta;
  ...
  while (!opened.empty()) {
    ...
    sort(opened.begin(), opened.end(),
        [](const HBF::maze_s& a, const HBF::maze_s& b) {
        return a.f < b.f;
        });
    ...
    for (int i = 0; i < next_state.size(); i++) {
      ...
      if (closed[stack2][idx(x2)][idx(y2)] == 0 && grid[idx(x2)][idx(y2)] == 0) {
        next_state[i].f = next_state[i].g + heuristic[idx(y2)][idx(x2)];
